        ...
        <div v-else>
          ...
          <template v-if="column.edit == true">
            <template v-if="scope.row.edit[column.value]">
              <el-input v-focus @blur="saveEdit(scope.row, column.value,scope)" v-model="scope.row[column.value]" class="edit-input" size="small"/>
            </template>
            <span v-else>{{ scope.row[column.value] }}</span><span style="opacity:0;" ></span>
          </template>
          <template v-else>
